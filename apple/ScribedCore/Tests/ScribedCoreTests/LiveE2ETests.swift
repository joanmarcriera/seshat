...
/// LAN-only end-to-end test. SKIPPED unless `SCRIBED_LIVE=1`. Endpoints are read
...
final class LiveE2ETests: XCTestCase {
    ...
    func testEndToEndAgainstLiveServers() async throws {
        ...
        try XCTSkipUnless(env["SCRIBED_LIVE"] == "1", "set SCRIBED_LIVE=1 to run the live harness")

        let audioPath = try XCTUnwrap(
            env["SCRIBED_LIVE_AUDIO"], "set SCRIBED_LIVE_AUDIO to a sample recording path")
        ...
        let wav = FileManager.default.temporaryDirectory
            .appendingPathComponent("scribed-live-\(UUID().uuidString).wav")
